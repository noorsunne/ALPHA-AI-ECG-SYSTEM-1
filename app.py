"""
╔══════════════════════════════════════════════════════════════════╗
║          ALPHA AI ECG SYSTEM — Merged ECG Analysis Platform     ║
║  Alpha ECG real processing  +  ALPHA UI  +  Groq AI Chat       ║
║                        Version 2.0                              ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="ALPHA AI ECG SYSTEM — ECG Analysis Platform",
    page_icon="🫀",
    layout="wide",
...
def chat_with_groq(client, user_message, chat_history, ecg_context):
    system_prompt = f"""You are Alpha AI ECG Assistant, an expert AI cardiologist assistant specializing in ECG interpretation.
...
def generate_pdf_report(patient_name, patient_age, patient_gender, patient_id,
    ...
    # Header
    header_data = [[
        Paragraph("<b>🫀 ALPHA AI ECG SYSTEM</b>", sty("hL", fontSize=18, fontName="Helvetica-Bold",
    ...
    story.append(Paragraph(
        ...
        "Always consult a qualified cardiologist for clinical ECG interpretation. ALPHA AI ECG SYSTEM v2.0",
        small_style
    ))
...
def show_admin_dashboard(user):
    ...
    <div class="cl-header">
      ...
      <p>Welcome, <b>{user['full_name']}</b> · ALPHA AI ECG SYSTEM Management Panel</p>
    </div>
    """, unsafe_allow_html=True)
...
def show_doctor_dashboard(user):
    ...
    <div class="cl-header">
      ...
      <p>Welcome, <b>{user['full_name']}</b> · {user.get('specialization','Cardiologist')} · ALPHA AI ECG SYSTEM</p>
    </div>
    """, unsafe_allow_html=True)
...
def show_patient_dashboard(user):
    ...
    <div class="cl-header">
      ...
      <p>Welcome, <b>{user['full_name']}</b> · Patient ID: {user.get('patient_id','—')} · ALPHA AI ECG SYSTEM</p>
    </div>
    """, unsafe_allow_html=True)
...
if user is None:
    ...
    with st.sidebar:
        st.markdown('<div class="sidebar-logo">🫀 <span>Alpha</span> AI ECG</div>', unsafe_allow_html=True)
        ...
        st.markdown("---")
        st.caption(f"ALPHA AI ECG SYSTEM v2.0\n{user['role'].title()} Portal")
        st.markdown("---")
...
# Footer
st.markdown("---")
st.caption("ALPHA AI ECG SYSTEM v2.0 · OpenCV + scipy + Groq LLaMA-3.3-70B + ReportLab · FYP Project 2025")
